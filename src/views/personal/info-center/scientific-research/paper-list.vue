  ...
  <div v-loading="loading" class="app-container" element-loading-text="数据查询中..."
    ...
    <el-form :inline="true" :model="queryForm">
      <el-form-item label="论文标题">
        <el-input v-model="queryForm.title" class="max-width-150" size="small" clearable placeholder="论文标题" />
      </el-form-item>
      ...
      <!-- <el-form-item label="级别">
                <el-select v-model="queryForm.level" :clearable="true" size="small" class="max-width-150" placeholder="级别">
      ...
      <el-form-item class="right">
        <el-button type="primary" size="small" icon="el-icon-plus" @click="toAdd(0)">添加科研论文</el-button>
      </el-form-item>
    </el-form>
    <el-table :header-cell-style="{ background: '#fafafa' }" :data="list" style="width: 100%">
      <el-table-column width="100px" type="index" :index="indexMethod" label="序号" />
      <el-table-column prop="title" label="论文标题" />
      <el-table-column prop="publish" label="刊物（出版社）名称" />
      <el-table-column prop="sci_count" label="SCI引用次数" />
      <el-table-column prop="factor" label="论文因子" />
      <el-table-column prop="rank" label="本人排名/总人数" />
      <el-table-column label="类型">
        <template slot-scope="scope">
          {{ paperType[scope.row.type] }}
        </template>
      </el-table-column>
      ...
      <el-table-column prop="time" label="发表年月" />
      ...
      <el-table-column fixed="right" width="180" label="操作">
        <template slot-scope="scope">
          ...
          <el-button v-if="scope.row.is_deleted === 0" size="mini" type="danger"
            @click="handleDelete(scope.row, 1)">删除</el-button>
          <el-button v-if="scope.row.is_deleted === 1" size="mini" type="info"
    ...
    <el-dialog title="附件" :visible.sync="dialogVisible">
      <div v-for="(item, index) in current.file_urls" :key="index">
        <el-link :href="domain + item" target="_blank">附件{{ index + 1 }}</el-link>
      </div>
      <empty v-if="current.file_urls.length === 0" description="未上传附件" />
    </el-dialog>
  </div>
</template>
<script>
