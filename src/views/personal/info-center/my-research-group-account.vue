  ...
  <div v-loading="loading" class="app-container" element-loading-text="数据查询中..."
    ...
    <el-table :header-cell-style="{ background: '#fafafa' }" :data="list" style="width: 100%">
      ...
      <el-table-column label="操作">
        <template slot-scope="scope">
          <el-button type="primary" size="small" @click="myConsume(scope.row)">我的消费记录</el-button>
        </template>
      </el-table-column>
      <div slot="empty">
        <empty />
    ...
    <el-dialog title="我的消费记录" :fullscreen="true" :visible.sync="dialogVisible">
      <el-form :inline="true">
        <el-form-item label="账号名称">
          {{ queryBillForm.research_group_account_name }}
        </el-form-item>
      </el-form>
      ...
      <el-pagination class="dl-pagination" :hide-on-single-page="true" background
        layout="total, prev, pager, next, jumper" :total="billTotal" :current-page="queryBillForm.page"
        :page-size="queryBillForm.size" @current-change="handleBillChange" />
    ...
    methods: {
      ...
      toAdd(id = '') {
        if (!id) {
          ...
          this.$router.push({
            path: '/personal/research-group-account-auth/edit/' + id,
          });
        }
      },
