  ...
  <div v-loading="loading" class="app-container" element-loading-text="数据处理中..."
    ...
    <el-form ref="form" :model="form" label-width="100px">
      <div style="border: 1px solid #efefef; border-radus: 10px; padding-top: 18px; margin: 18px 0">
        ...
        <el-form-item label="申请采购耗材">
          <el-transfer style="text-align: left; display: inline-block" filterable v-model="reagent"
            :left-default-checked="reagentLeft" :right-default-checked="[5]" :titles="['未添加', '已添加']"
            :button-texts="['取消', '添加']" :format="{
              noChecked: '${total}',
              hasChecked: '${checked}/${total}',
            }" @change="reagentChange" :data="reagentData">
      ...
      <!-- <div style="border: 1px solid #efefef; border-radus: 10px; padding-top: 18px; margin: 18px 0;">
                ...
                <el-form-item v-if="hcs_ids_arr.length > 0" label="危化品">
                    <div v-for="(item, index) in hcs_ids_arr" :key="index">
                        {{ item.label }}：<el-input-number @change="(currentValue, oldValue) => { numberChange(currentValue, oldValue, index, 2) }" size="small" :min="1" :step="1" v-model="item.num"></el-input-number>
                    </div>
                </el-form-item>
            </div> -->
      ...
      <el-form-item label="备注">
        <el-input type="textarea" rows="3" size="small" class="max-width-300" v-model="form.note"></el-input>
      </el-form-item>
      <el-form-item>
    ...
    methods: {
      ...
      getPrice() {
        ...
        if (this.hcs_ids_arr.length > 0) {
          ...
          countForm.hcs_default = JSON.stringify(hcs_ids_arr);
        }
        this.loading = true;
