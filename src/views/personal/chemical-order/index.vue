  ...
  >
    ...
    <el-table :header-cell-style="{ background: '#fafafa' }" :data="list" style="width: 100%">
      ...
      <el-table-column label="状态">
        <template slot-scope="scope">
          <el-tag v-if="scope.row.platform_audit === 0" type="default">待实验室审核</el-tag>
          <el-tag v-if="scope.row.platform_audit === -1" type="danger">实验室未通过</el-tag>
...
export default {
  ...
  methods: {
    ...
    toAdd(id = '') {
      if (!id) {
        this.$router.push({ path: '/personal/chemical-order/add' });
      } else {
        this.$router.push({
