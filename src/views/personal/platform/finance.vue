  ...
  >
    <el-card class="finance">
      <div slot="header" class="clearfix">
        <el-form :inline="true">
          <el-form-item label="实验室名称" style="margin-bottom: 0 !important">{{
            platform.name
          }}</el-form-item>
          <el-form-item
            label="余额"
    ...
    <!-- <el-form :inline="true">
            <el-form-item label="实验室名称">{{ platform.name }}</el-form-item>
...
export default {
  ...
  methods: {
    getInfo() {
      platformInfo({ platform_id: this.$route.params.id }).then((response) => {
        if (response.status !== 200) {
          this.returnToList();
        } else {
          this.platform = response.data;
          this.queryForm.id = this.platform.id;
          financeStatistics({
            type: 2,
            id: this.platform.id,
          }).then((res) => {
    ...
    returnToList() {
      this.$router.push({ path: '/personal/platform/finance-list' });
    },
    indexMethod(index) {
