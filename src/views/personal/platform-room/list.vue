  ...
  >
    ...
    <el-table :header-cell-style="{ background: '#fafafa' }" :data="list" style="width: 100%">
      ...
      <el-table-column fixed="right" width="220" label="操作">
        <template slot-scope="scope">
          <el-button size="mini" @click="toOpenDoor(scope.row.id)">开门</el-button>
...
export default {
  ...
  methods: {
    queryList() {
      this.loading = true;
      frontPlatformRoomList(this.queryForm)
        .then((res) => {
          this.loading = false;
          if (res.status === 200) {
    ...
    handleDelete(row, status) {
      this.$confirm('确定要删除该门禁吗？', '提示', {
        ...
        deletePlatformRoom({ id: row.id, is_deleted: status }).then((response) => {
    ...
    toOpenDoor(id) {
      ...
      openDoor({ id }).then((res) => {
        loading.close();
      });
    },
    toAdd(id = '') {
      if (!id) {
        this.$router.push({ path: '/personal/platform-room/add' });
      } else {
        this.$router.push({
