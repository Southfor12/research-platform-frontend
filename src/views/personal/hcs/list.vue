<template>
  ...
  >
    <el-form :inline="true" :model="queryForm">
      ...
      <el-form-item v-if="checkNode('add_hcs')" class="right">
    ...
    <el-table :header-cell-style="{ background: '#fafafa' }" :data="list" style="width: 100%">
      ...
      <el-table-column prop="admin1_name" label="负责人1" />
      <el-table-column prop="admin2_name" label="负责人2" />
      ...
      <el-table-column
        v-if="checkNode('edit_hcs') || checkNode('delete_hcs')"
        width="180"
        label="操作"
      >
        <template slot-scope="scope">
          <el-button
            v-if="checkNode('edit_hcs')"
            size="mini"
            type="primary"
          ...
          >
          <el-button
            v-if="checkNode('delete_hcs') && scope.row.is_deleted === 0"
            size="mini"
            type="danger"
          ...
          >
          <el-button
            v-if="checkNode('delete_hcs') && scope.row.is_deleted === 1"
            size="mini"
            type="info"
...
<script>
import { hcsList, deleteHcs } from '@/api/hcs';
  ...
  methods: {
    ...
    handleDelete(row, status) {
      ...
      this.$confirm('确定要' + text + '"' + row.name + '"吗？', '提示', {
        ...
        deleteHcs({ id: row.id, is_deleted: status }).then((response) => {
    ...
    toAdd(id = '') {
      if (!id) {
        this.$router.push({ path: '/personal/hcs/add' });
      } else {
        this.$router.push({ path: '/personal/hcs/edit/' + id });
      }
    },
