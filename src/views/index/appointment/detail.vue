  ...
  <div>
    <ul class="info">
      <li style="text-align: left; min-height: 300px">
        ...
        <el-table :data="apparatusLeftProperties" style="width: 100%" stripe size="medium" :show-header="false">
      ...
      <li style="width: 48%; display: table; padding-top: 8px">
        ...
        <el-table :data="apparatusRightProperties" style="width: 100%; margin-left: 0px" stripe size="medium"
          :show-header="false">
    ...
    <el-tabs @tab-click="tabClick" v-model="activeName" type="border-card" class="index-common-table-container">
      ...
      <el-tab-pane v-if="
        ...
        <div class="tab-container">
          ...
          <el-radio-group v-if="member.id" v-model="form.type" style="margin-bottom: 30px; margin-left: 30px"
            @change="checkForAppointment">
            <el-radio-button v-if="apparatus.share_type.split(',').indexOf('1') > -1" label="1">自主检测</el-radio-button>
            <el-radio-button v-if="apparatus.share_type.split(',').indexOf('2') > -1" label="2">送样检测</el-radio-button>
          </el-radio-group>
          <!-- 选择日期的日历表 -->
          <full-calendar v-show="showMonth && member.id" id="schedulerCalendar" ref="calendar"
            style="margin-bottom: 30px" :events="events" :config="config"></full-calendar>
          <el-form v-loading="loading" element-loading-text="正在计算费用..." element-loading-spinner="el-icon-loading"
            v-show="member.id" ref="form" :model="form" :rules="rules" label-width="150px">
            <div v-show="!showMonth && member.id">
              <div class="full-calendar-container">
                <full-calendar id="agendaDayCalendar" ref="agendaDayCalendar" :events="agendaDayEvents"
                  :config="agendaDayConfig"></full-calendar>
                <div style="position: absolute; right: 10px; top: 5px; cursor: pointer; color: #2068b0"
                  @click="reSelectDate">
              ...
              <el-row>
                <el-col :span="10">
                  <el-form-item label="样品名称" prop="sample_name">
                    <el-input class="width-200" type="text" :min="0" v-model="form.sample_name" size="small"
                      placeholder="请填写样品名称" />
                  </el-form-item>
                </el-col>
                <el-col :span="14">
                  <el-form-item label="样品数量" prop="sample_num">
                    <el-input-number v-model="form.sample_num" class="width-200" @change="
                        (currentValue, oldValue) => {
                          priceNumberChange(currentValue, oldValue, 0, 1);
                        }
                      " size="small" :min="0" :step="1" :precision="0">
                    </el-input-number>
                  </el-form-item>
                </el-col>
              </el-row>
                ...
                <el-table :data="chargeList" stripe style="width: 95%">
                  <el-table-column prop="title" label="分类(带星号的必须选)" width="200">
                    <template slot-scope="scope">
                      <span style="color: #ff4949; font-size: 16px; font-weight: 500; margin-right: 4px"
                        v-if="scope.row.choose">*</span>
                      {{ scope.row.title }}
                    </template>
                  </el-table-column>
                  <el-table-column prop="id" label="检测项目(请勾选需要做检测的项目)">
                    <template slot-scope="scope">
                      <el-checkbox-group @change="
                        ...
                        <el-checkbox style="display: block" v-for="(typeItem, typeIndex) in scope.row.items"
                          :key="typeIndex" :label="typeItem.id" :checked="scope.row.choose == 1">
                          ...
                          <span v-if="typeItem.note">{{ typeItem.note }}</span><span
                            v-if="typeItem.self_free">(自主检测免费)</span>
                        </el-checkbox>
                      </el-checkbox-group>
                    </template>
                  </el-table-column>
              ...
              <el-row>
                <el-col :span="14">
                  <el-form-item label="付款账号" prop="research_group_account_id">
                    <el-select v-model="form.research_group_account_id" size="small" @change="accountChange"
                      class="width-300" filterable reserve-keyword placeholder="请选择付款账号">
                      <el-option v-for="item in accounts" :key="item.research_group_account_id"
                        :label="item.research_group_account_name" :value="item.research_group_account_id" />
                    </el-select>
                  </el-form-item>
                </el-col>
              </el-row>
              ...
              <el-form-item v-if="form.type == 2" label="报告类型">
                <el-select v-model="report_type" @change="reportTypeChange" size="small" placeholder="请选择"
                  class="width-200">
                  <el-option label="原始数据" :value="0" />
                  <el-option label="数据报告" :value="1" />
                </el-select>
                <el-select v-if="report_type == 1" @change="
                    (value) => {
                      countBookCharge();
                    }
                  " v-model="form.report_type" size="small" placeholder="请选择">
              ...
              <el-form-item v-if="form.type == 2" label="报告发送方式">
                <el-select v-model="form.reposrt_send_type" size="small" placeholder="请选择" class="width-200">
                  <el-option label="自取" :value="1" />
                  <el-option v-if="!member.is_inner" label="寄送" :value="2" />
                </el-select>
              </el-form-item>
              <el-form-item label="检测要求" prop="note">
                <el-input type="textarea" class="width-500" :rows="5" v-model="form.note" size="small" />
              </el-form-item>
            </div>
            <div v-if="!showMonth && member.id">
              <el-card v-if="reagent.length > 0 && form.type == 1" class="common-box-card index-common-table-container">
                    ...
                    <el-table-column label="预约使用量">
                      <template slot-scope="scope">
                        <el-input-number @change="
                          ...
                          " size="small" :min="0" :step="1" :precision="0"
                          v-model="scope.row.price_number_num"></el-input-number>
                      </template>
                    </el-table-column>
                  </el-table>
                </div>
              </el-card>
              <el-form-item v-if="form.type === '1' && 0" label="是否自带耗材">
                <el-select v-model="form.is_carry_reagents" size="small" class="width-200" filterable reserve-keyword>
                  <el-option label="是" value="1"></el-option>
                  <el-option label="否" value="0"></el-option>
                </el-select>
              </el-form-item>
              <el-form-item v-if="form.type === '1' && form.is_carry_reagents === '1'" label="自带耗材"
                prop="carry_reagents_arr">
                <el-select v-model="form.carry_reagents_arr" size="small" class="width-200" allow-create multiple
                  filterable default-first-option>
                </el-select>
              </el-form-item>
              <el-card v-if="hcses.length > 0 && form.type == 1" class="common-box-card index-common-table-container">
                <div slot="header" class="index-box-card-header">
                  <div>
                    危化品 (<span class="hcs-process" @click="showProcess = true">危化品在线领用流程</span>)
                  </div>
                </div>
                <div class="tab-container">
                    ...
                    <el-table-column label="等级">
                      <template slot-scope="scope">
                        <el-select size="small" v-model="scope.row.level" placeholder="请选择等级">
                          <el-option v-for="item in scope.row.levels" :key="item" :label="item" :value="item">
                          </el-option>
                        </el-select>
                      </template>
                    </el-table-column>
                    <el-table-column label="规格">
                      <template slot-scope="scope">
                        <el-select size="small" v-model="scope.row.specification" placeholder="请选择规格">
                          <el-option v-for="item in scope.row.specifications" :key="item" :label="item" :value="item">
                          </el-option>
                        </el-select>
                      </template>
                    </el-table-column>
              ...
              <el-form-item v-if="form.type === '1' && 0" label="是否自带危化品">
                <el-select v-model="form.is_carry_hcses" size="small" class="width-200" filterable reserve-keyword>
                  <el-option label="是" value="1"></el-option>
                  <el-option label="否" value="0"></el-option>
                </el-select>
              </el-form-item>
              <el-form-item v-if="form.type === '1' && form.is_carry_hcses === '1'" label="自带危化品"
                prop="carry_hcses_arr">
                <el-select v-model="form.carry_hcses_arr" size="small" class="width-200" allow-create multiple
                  filterable default-first-option>
                </el-select>
              </el-form-item>
            </div>
    ...
    <el-dialog title="说明书" :close-on-click-modal="false" :show-close="false" :visible.sync="dialogVisible">
    ...
    <el-dialog title="危化品在线领用流程" :close-on-click-modal="false" :show-close="false" :visible.sync="showProcess">
      <div v-html="hcsStatement"></div>
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" @click="showProcess = false">确定</el-button>
      </span>
    </el-dialog>
    <el-dialog :title="`仪器 [${apparatus.name}] 使用注意事项`" :close-on-click-modal="false" :show-close="false"
    ...
    <el-dialog title="仪器使用考试" @close="clearAnswer" :close-on-click-modal="false" :show-close="false"
      ...
      <div>
        ...
        <div class="question-answer single-question" v-for="(item, index) in singleChoose" :key="item.id">
          <div class="question-title">{{ index + 1 }}. {{ item.name }}</div>
          <div>
            <el-radio-group v-model="item.answer">
              <el-radio v-for="item1 in item.items" v-show="item1.content" :key="item1.name" :label="item1.name">
                <div class="question-answer-item" style="display: inline-block">
                  {{ item1.name }}.
                </div>
                <div class="question-answer-content" style="display: inline-block; white-space: normal">
                  {{ item1.content }}
                </div>
              </el-radio>
            </el-radio-group>
        ...
        <div class="question-answer single-question" v-for="(item, index) in multiChoose" :key="item.id">
          <div class="question-title">{{ index + 1 }}. {{ item.name }}</div>
          <div>
            <el-checkbox-group v-model="item.answer">
              <el-checkbox v-for="item1 in item.items" v-show="item1.content" :key="item1.name"
                :label="item1.name">{{ item1.name }}. {{ item1.content }}</el-checkbox>
            </el-checkbox-group>
          </div>
        </div>
      </div>
  ...
  export default {
            ...
            if (jsEvent.type === 'month') {
              ...
              if (!canSelect) {
                ...
                if (that.booking_limit == 1) {
                  ...
                  // 比较当前时间是否在指定范围之间
                  if (currentFormattedTime >= a && currentFormattedTime <= b) {} else {
                    that.$message.error(
        ...
        agendaDayConfig: {
          ...
          select: function(startDate, endDate, allDay, jsEvent, view) {
            ...
            if (jsEvent.type === 'agendaDay') {
              ...
              do {
                mergeOk = false;
                for (let i = 0; i < that.chosedTimes.length; i++) {
                  let a = that.chosedTimes[i];
                  for (let j = i + 1; j < that.chosedTimes.length; j++) {
                    let b = that.chosedTimes[j];

                    if (a.start == b.end) {
                      //合并a和b
                      //把A合并到B上
                      that.chosedTimes[j].end = a.end;
                      that.chosedTimes[j].endDate = a.endDate;
                      mergeOk = true;
                      that.chosedTimes.splice(i, 1);
                      break;
                    } else if (a.end == b.start) {
                      //合并a和b
                      //把B合并到A上
                      that.chosedTimes[i].end = b.end;
                      that.chosedTimes[i].endDate = b.endDate;
                      mergeOk = true;
                      //删除B
                      that.chosedTimes.splice(j, 1);
                      break;
                    }
                  }
                  if (mergeOk) {
              ...
              that.agendaDayEvents = that.agendaDayEvents.filter(function(item) {
                return item.flag != 'book_time';
              });
              ...
              for (let i = 0; i < that.chosedTimes.length; i++) {
                that.agendaDayEvents.push({
                  ...
                  resourceId: i + 1,
                  title: '已选择时间段 (点击取消选择)',
                  start: that.chosedTimes[i].startDate,
                  end: that.chosedTimes[i].endDate,
                  color: 'blue',
                  textColor: '#ffffff',
                  editable: false,
                  selectable: false,
    ...
    methods: {
      ...
      countBookCharge() {
        ...
        this.countForm.booking_times = this.chosedTimes;
            ...
            for (let date in this.bookableTime.booked_time) {
              if (
                ...
                this.bookableTime.booked_time[date].forEach((item, index) => {
                  let title = '预约人: ' + item.creator_name; //院内人员不显示手机号
                  if (item.is_inner == 0) {
                    title = '预约人: ' + item.creator_name + ' ' + item.creator_mobile; //院外人员显示手机号
                  }
