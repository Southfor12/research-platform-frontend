  ...
  <div class="register-container">
      <div class="common-header">
          ...
          <div class="common-header__login">
              <div>
                  <a href="#" @click="navTo('/login')" :underline="false">登录</a>
              </div>
          </div>
      </div>
      <div class="register-form-container">
          ...
          <el-form ref="form" :model="form" :rules="rules" label-width="80px">
              ...
              <el-form-item>
                  <el-button :loading="loading" type="primary" @click.native.prevent="onSubmit" class="submitBtn">提交</el-button>
              </el-form-item>
          </el-form>
      </div>
  </div>
      ...
      return {
          ...
          rules: {
              ...
              code: [
                  ...
                  { min: 6, max: 6, message: '请正确填写6位验证码', trigger: 'blur' }
              ],
              password: [
                  { required: true, message: '请填写密码', trigger: 'blur' },
                  { pattern: /^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[^a-zA-Z0-9]).{8,16}$/, message: '密码必须由大小写字母、数字、特殊字符组成的8-16字符组成', trigger: 'blur' }
              ],
              rePassword: [
  ...
  methods: {
      onSubmit() {
          this.$refs.form.validate(valid => {
              if (valid) {
                  this.loading = true
                  resetPassword({ mobile: this.form.mobile, password: sha256(this.form.password),
                      rePassword: sha256(this.form.rePassword), code: this.form.code }).then((res) => {
                      this.loading = false
                      if (res.status === 1) {
                        this.navTo('/');
                      }
