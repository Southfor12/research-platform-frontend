<template>
  <div>
    <van-sticky>
      <van-nav-bar title="学生离室审核" left-text="返回" left-arrow @click-left="returnTo" />
      <van-dropdown-menu>
        <van-dropdown-item
          @change="resetQuery"
...
<script>
import { mentorAuditLeaveList, mentorAuditLeave } from '@/api/platform';
...
export default {
  ...
  methods: {
    ...
    queryList() {
      this.loading = true;
      mentorAuditLeaveList(this.queryForm)
        .then((res) => {
          this.loading = false;
          if (res.status === 200) {
      ...
      if (this.auditForm.status === -1) {
        this.$refs[form].validate((valid) => {
          if (valid) {
            ...
            mentorAuditLeave(this.auditForm).then((response) => {
