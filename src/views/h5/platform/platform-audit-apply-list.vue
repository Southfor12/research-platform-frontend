<template>
  <div>
    <van-sticky>
      <van-nav-bar title="入室审核" left-text="返回" left-arrow @click-left="returnTo" />
      <van-dropdown-menu>
        <van-dropdown-item
          @change="resetQuery"
      ...
      <div>
        ...
        >
          <div slot="footer" class="book-footer">
            ...
            >
            <van-button
              v-if="item.mentor_audit && item.pass_exam && item.platform_audit === 0"
              size="small"
              type="primary"
...
<script>
import { frontApplyPlatformList, platformAudit } from '@/api/platform';
...
export default {
  ...
  methods: {
    ...
    queryList() {
      this.loading = true;
      frontApplyPlatformList(this.queryForm)
        .then((res) => {
          this.loading = false;
          if (res.status === 200) {
      ...
      if (this.auditForm.status === -1) {
        this.$refs[form].validate((valid) => {
          if (valid) {
            ...
            platformAudit(this.auditForm).then((response) => {
