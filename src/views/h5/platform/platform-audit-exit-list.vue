<template>
  <div>
    <van-sticky>
      <van-nav-bar title="离室审核" left-text="返回" left-arrow @click-left="returnTo" />
      <van-dropdown-menu>
        <van-dropdown-item
          @change="resetQuery"
          v-model="queryForm.platform_audit"
    ...
    <van-list v-model="loading" :finished="finished" finished-text="没有更多了" @load="queryList">
      <div>
        ...
        >
          <div
            v-if="item.platform_audit === 0 && item.mentor_audit && item.platform_audit === 0"
            slot="footer"
            class="book-footer"
          >
            <van-button size="small" type="primary" @click="handleAudit(item, 1)">通过</van-button>
            <van-button size="small" type="danger" @click="handleAudit(item, -1)">拒绝</van-button>
          </div>
        </van-panel>
      </div>
    </van-list>
...
<script>
import { frontApplyLeavePlatformList, platformAuditLeave } from '@/api/platform';
...
export default {
  ...
  methods: {
    resetQuery(value) {
      ...
      this.queryForm.platform_audit = value;
      this.list = [];
      this.finished = false;
      this.queryList();
    },
