  ...
  <div>
    <van-list v-model="loading" :finished="finished" finished-text="没有更多了" @load="queryList">
      ...
      <van-panel
        ...
        :key="index"
        :title="'预约人：' + item.creator_name + ' ' + item.user_mobile"
        :desc="'检测要求：' + (item.note ? item.note : '-')"
        :class="{
      ...
      >
        <div class="book-container">
          <div class="book-item">
            实验操作人： <span>{{ item.user_name }}</span>
          </div>
          <div class="book-item">
            预约设备：
            <span>{{ item.apparatus_name }}（管理员：{{ item.apparatus_admin_name }}）</span>
          </div>
          <div class="book-item">
            预约类型：
          ...
          <div class="book-item">
            样本数量： <span>{{ item.sample_num }}份</span>
          </div>
          ...
          <div class="book-item">
            预约时间：
            <span v-html="item.booking_time" class="book-time"></span>
          </div>
          <div class="book-item">
          ...
          <!-- <div v-if="item.type === 2" class="book-item">期望完成时间： <span>{{ item.expected_finish_date }}</span></div> -->
          <van-collapse v-model="activeNames">
            ...
            <!-- <van-collapse-item
                                ...
                                <el-form label-width="100px">
                                    ...
                                    <el-form-item label="发票抬头">{{ item.invoice.head ? item.invoice.head : '-' }}</el-form-item>
                                    <el-form-item label="纳税人识别号">{{ item.invoice.no ? item.invoice.no : '-' }}</el-form-item>
                                    <el-form-item label="开户银行">{{ item.invoice.bank ? item.invoice.bank : '-' }}</el-form-item>
                                    <el-form-item label="银行账户">{{ item.invoice.bank_no ? item.invoice.bank_no : '-' }}</el-form-item>
                                    <el-form-item label="联系地址">{{ item.invoice.address ? item.invoice.address : '-' }}</el-form-item>
                                    <el-form-item label="联系电话">{{ item.invoice.mobile ? item.invoice.mobile : '-' }}</el-form-item>
                                </el-form>
                            </van-collapse-item> -->
          </van-collapse>
        </div>
        <div slot="footer" class="book-footer">
          ...
          <van-button
            v-if="checkNode('booking_examine') && item.status === 0"
            size="small"
            type="primary"
          ...
          <!-- <van-button v-if="item.user_id === member.id && item.status < 2 && item.is_charge !== 1" size="small" @click="handleCancleBooking(item)">取消预约</van-button> -->
        </div>
      </van-panel>
    </van-list>
    <van-dialog
...
<script>
import { frontBookingList, bookableUpdate } from '@/api/apparatus';
