<template>
  ...
  >
    <el-form :inline="true" :model="queryForm">
      ...
      <el-form-item
        v-if="checkNode('add_apparatus_exam_question') || checkNode('import_appratus_exam')"
        class="right"
      >
       <!-- <el-button
          v-if="checkNode('import_appratus_exam')"
          type="primary"
          size="small"
          icon="el-icon-plus"
        ...
        > -->
        <el-button
          v-if="checkNode('add_apparatus_exam_question')"
          type="primary"
          size="small"
          icon="el-icon-plus"
          @click="toAdd(0)"
    ...
    <el-table :header-cell-style="{ background: '#fafafa' }" :data="list" style="width: 100%">
      ...
      <el-table-column
        fixed="right"
        v-if="
          checkNode('edit_apparatus_exam_question') || checkNode('delete_apparatus_exam_question')
        "
        width="180"
        label="操作"
      >
        <template slot-scope="scope">
          <el-button
            v-if="checkNode('edit_apparatus_exam_question')"
            size="mini"
            type="primary"
          ...
          >
          <el-button
            v-if="checkNode('delete_apparatus_exam_question') && scope.row.is_deleted === 0"
            size="mini"
            type="danger"
          ...
          >
          <el-button
            v-if="checkNode('delete_apparatus_exam_question') && scope.row.is_deleted === 1"
            size="mini"
            type="info"
    ...
    <el-dialog title="批量导入题目" :visible.sync="importVisible">
      <el-form ref="importForm" :rules="rules" label-width="150px" :model="importForm">
        <el-form-item label="仪器" prop="apparatus_id">
          <el-select
            v-model="importForm.apparatus_id"
            size="small"
            clearable
            class="max-width-150"
            filterable
...
export default {
  ...
  methods: {
    ...
    toAdd(id = '') {
      if (!id) {
        this.$router.push({ path: '/apparatus-exam-question/add' });
      } else {
        this.$router.push({
