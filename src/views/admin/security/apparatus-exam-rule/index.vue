<template>
  <div v-loading="loading" class="app-container" element-loading-text="数据查询中..."
    ...
    <el-table :header-cell-style="{ background: '#fafafa' }" :data="list" style="width: 100%">
      ...
      <el-table-column fixed="right" v-if="checkNode('edit_apparatus_exam_rule')" width="180" label="操作">
        <template slot-scope="scope">
          <el-button v-if="checkNode('edit_apparatus_exam_rule')" size="mini" type="primary"
            @click="toAdd(scope.row.id)">修改仪器考试规则</el-button>
        </template>
      </el-table-column>
      <div slot="empty">
        <empty />
