  ...
  >
    ...
    <el-table :header-cell-style="{ background: '#fafafa' }" :data="list" style="width: 100%">
      ...
      <el-table-column width="180" label="操作">
        <template slot-scope="scope">
          <el-form :inline="true">
            <!-- <el-form-item v-if="scope.row.user_id === member.id && scope.row.status == '1'" style="margin-bottom: 0;">
                        <el-button
                            size="mini"
                            type="success"
                            @click="toAppend(scope.row)"
                        >补充申请</el-button>
                    </el-form-item> -->
            ...
            <el-form-item
              v-if="
                checkNode('platform_leave_apply_audit') &&
                scope.row.mentor_audit &&
                scope.row.platform_audit &&
                scope.row.status === 0
              "
      ...
      <div v-else>
        ...
        <el-form label-width="100px">
          <el-form-item label="欠款明细">
            ...
            <div v-if="debtList.length === 0">无任何欠款</div>
          </el-form-item>
...
import {
  platformList,
  applyLeavePlatformList,
  deleteApplyPlatform,
  auditApplyPlatformLeave,
  applyPlatformAppend,
} from '@/api/platform';
...
export default {
  ...
  methods: {
    ...
    queryList() {
      this.loading = true;
      applyLeavePlatformList(this.queryForm)
        .then((res) => {
          this.loading = false;
          if (res.status === 200) {
    ...
    comfirmAudit(form) {
      if (this.auditForm.status == '-1') {
        this.$refs[form].validate((valid) => {
          if (valid) {
            auditApplyPlatformLeave(this.auditForm).then((response) => {
