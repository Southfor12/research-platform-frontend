<template>
  <div class="container">
    ...
    <el-table :data="animalData" border style="width: 100%" v-loading="loading" class="custom-table">
      ...
      <el-table-column prop="name" label="动物状态" align="center" />
      <el-table-column prop="code" label="状态标识" align="center" />
    ...
    <!-- 新建/编辑对话框 -->
    <el-dialog :title="dialogType === 'edit' ? '编辑动物状态' : '新建动物状态'" :visible.sync="dialogVisible" width="500px">
      <el-form :model="form" :rules="rules" ref="form" label-width="120px">
        <el-form-item label="动物状态名称" prop="name">
...
export default {
  data() {
    return {
      ...
      rules: {
        name: [
          { required: true, message: '请输入动物状态名称', trigger: 'blur' }
        ],
        code: [
          { required: true, message: '请输入动物状态标识', trigger: 'blur' },
          { pattern: /^[A-Z_]+$/, message: '标识需全大写字母和下划线', trigger: 'blur' }
        ]
      }
    }
  },
  ...
  methods: {
    ...
    async submitForm() {
      try {
        ...
        if (this.dialogType === 'edit') {
          ...
          await editAnimalStatus(params)
        } else {
          await addAnimalStatus(params)
        }
    ...
    async handleDelete(row) {
      try {
        ...
        const res = await delAnimalStatus({ id: row.id })
        if (res.status === 1) {
    ...
    async handleSearch() {
      ...
      try {
        ...
        if (keyword) {
          //调用搜索接口
          const res = await getAnimalStatusByName({
            status: keyword
          })
          if (res.status === 1) {
