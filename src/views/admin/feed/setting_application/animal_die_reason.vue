<template>
  <div class="container">
    ...
    <el-table :data="animalData" border style="width: 100%" v-loading="loading" class="custom-table">
      ...
      <el-table-column prop="name" label="处死原因" align="center" />
    ...
    <!-- 新建/编辑对话框 -->
    <el-dialog :title="dialogType === 'edit' ? '编辑处死原因' : '新建处死原因'" :visible.sync="dialogVisible" width="500px">
      <el-form :model="form" :rules="rules" ref="form" label-width="120px">
        <el-form-item label="处死原因" prop="name">
          <el-input v-model="form.name" placeholder="请输入处死原因"></el-input>
        </el-form-item>
      </el-form>
...
<script>
import {
  getAnimalDeathReason,
  addAnimalDeathReason,
  delAnimalDeathReason,
  editAnimalDeathReason,
  getAnimalDeathReasonByName
...
export default {
  data() {
    return {
      ...
      rules: {
        name: [
          { required: true, message: '请输入处死原因', trigger: 'blur' }
        ],
  ...
  methods: {
    ...
    async submitForm() {
      try {
        ...
        if (this.dialogType === 'edit') {
          ...
          await editAnimalDeathReason(params)
        } else {
          await addAnimalDeathReason(params)
        }
    ...
    async handleDelete(row) {
      try {
        ...
        const res = await delAnimalDeathReason({ id: row.id })
        if (res.status === 1) {
    ...
    async handleSearch() {
      ...
      try {
        ...
        if (keyword) {
          //调用搜索接口
          const res = await getAnimalDeathReasonByName({
            name: keyword
          })
          if (res.status === 1) {
          ...
          //空关键字时加载全部数据
          await this.getAnimalDeathReason()
        }
      }
      catch (error) {
