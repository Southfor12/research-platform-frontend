  ...
  >
    ...
    <el-table :header-cell-style="{ background: '#fafafa' }" :data="list" style="width: 100%">
      ...
      <el-table-column label="操作">
        <template slot-scope="scope">
          <div v-if="checkNode('group_grade_list')" style="margin-top: 10px">
    ...
    />
    <el-dialog
      :title="queryScoreForm.group_name + '的评分'"
      :modal="false"
    ...
    >
      <grade
        ...
        :target_type="3"
        :target_id="queryScoreForm.group_id"
        :source_type="1"
        :source_id="0"
        :group_name="queryScoreForm.group_name"
        :add_auth="1"
        :edit_auth="1"
        :delete_auth="1"
      />
...
<script>
import { researchGroupList, deleteResearchGroup, applyJoinResearchGroup } from '@/api/research';
...
export default {
  ...
  methods: {
    queryList() {
      this.loading = true;
      researchGroupList(this.queryForm)
        .then((res) => {
          this.loading = false;
          if (res.status === 200) {
