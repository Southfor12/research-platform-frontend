<template>
  ...
  >
    ...
    <el-table :header-cell-style="{ background: '#fafafa' }" :data="list" style="width: 100%">
      ...
      <el-table-column prop="company" label="授予单位" />
      ...
      <el-table-column prop="time" label="授予年月" />
      <el-table-column prop="factory_id" label="单位">
          <template slot-scope="scope">
            {{getFactoryName(scope.row.factory_id)}}
          </template>
      </el-table-column>
      <el-table-column v-if="checkNode('sci_honour_audit')" width="220" label="操作">
        <template slot-scope="scope">
          <el-button
...
<script>
import { sciHonourList, deleteSciHonour, auditSciHonour } from '@/api/scientific';
  ...
  methods: {
    ...
    handleDelete(row, status) {
      ...
      this.$confirm('确定要' + text + '"' + row.title + '"吗？', '提示', {
        ...
        deleteSciHonour({ id: row.id, is_deleted: status }).then((response) => {
    ...
    handleAudit(row, status) {
      ...
      this.$confirm('确定要' + text + '吗？', '提示', {
        ...
        auditSciHonour({ id: row.id, status: status }).then((response) => {
          if (response.status === 1) {
            this.queryList();
          }
        });
    ...
    toAdd(id = '') {
      if (!id) {
        this.$router.push({ path: '/info-center/honour/add' });
      } else {
        this.$router.push({ path: '/info-center/honour/edit/' + id });
      }
    },
