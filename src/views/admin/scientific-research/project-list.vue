<template>
  ...
  >
    ...
    <el-table :header-cell-style="{ background: '#fafafa' }" :data="list" style="width: 100%">
      ...
      <el-table-column v-if="checkNode('sci_project_audit')" width="220" label="操作">
        <template slot-scope="scope">
          <el-button
...
<script>
import { sciProjectList, deleteSciProject, auditSciProject } from '@/api/scientific';
  ...
  methods: {
    ...
    handleDelete(row, status) {
      ...
      this.$confirm('确定要' + text + '"' + row.title + '"吗？', '提示', {
        ...
        deleteSciProject({ id: row.id, is_deleted: status }).then((response) => {
    ...
    handleAudit(row, status) {
      ...
      this.$confirm('确定要' + text + '吗？', '提示', {
        ...
        auditSciProject({ id: row.id, status: status }).then((response) => {
          if (response.status === 1) {
            this.queryList();
          }
        });
    ...
    toAdd(id = '') {
      if (!id) {
        this.$router.push({ path: '/info-center/project/add' });
      } else {
        this.$router.push({ path: '/info-center/project/edit/' + id });
      }
    },
