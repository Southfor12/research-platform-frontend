    ...
    <el-table :header-cell-style="{ background: '#fafafa' }" :data="list" style="width: 100%">
      ...
      <el-table-column fixed="right" width="180" label="操作">
        <template slot-scope="scope">
          <el-button
            v-if="
              checkNode('sci_paper_audit') && (scope.row.status === 1 || scope.row.status === 0)
            "
            size="mini"
            type="error"
...
<script>
import { sciPaperList, deleteSciPaper, auditSciPaper } from '@/api/scientific';
  ...
  methods: {
    ...
    handleDelete(row, status) {
      ...
      this.$confirm('确定要' + text + '"' + row.title + '"吗？', '提示', {
        ...
        deleteSciPaper({ id: row.id, is_deleted: status }).then((response) => {
    ...
    handleAudit(row, status) {
      ...
      this.$confirm('确定要' + text + '吗？', '提示', {
        ...
        auditSciPaper({ id: row.id, status: status }).then((response) => {
          if (response.status === 1) {
            this.queryList();
          }
        });
    ...
    toAdd(id = '') {
      if (!id) {
        this.$router.push({ path: '/info-center/paper/add' });
      } else {
        this.$router.push({ path: '/info-center/paper/edit/' + id });
      }
    },
