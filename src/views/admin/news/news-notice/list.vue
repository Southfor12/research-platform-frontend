<template>
  ...
  >
    <el-form :inline="true" :model="queryForm">
      ...
      <el-form-item v-if="checkNode('add_news_notice')" class="right">
    ...
    <el-table :header-cell-style="{ background: '#fafafa' }" :data="list" style="width: 100%">
      ...
      <el-table-column
        v-if="checkNode('edit_news_notice') || checkNode('delete_news_notice')"
        width="180"
        label="操作"
      >
        <template slot-scope="scope">
          <el-button
            v-if="checkNode('edit_news_notice')"
            size="mini"
            type="primary"
          ...
          >
          <el-button
            v-if="checkNode('delete_news_notice') && scope.row.is_deleted === 0"
            size="mini"
            type="danger"
          ...
          >
          <el-button
            v-if="checkNode('delete_news_notice') && scope.row.is_deleted === 1"
            size="mini"
            type="info"
...
export default {
  ...
  methods: {
    ...
    toAdd(id = '') {
      if (!id) {
        this.$router.push({ path: '/news-notice/add' });
      } else {
        this.$router.push({ path: '/news-notice/edit/' + id });
      }
    },
