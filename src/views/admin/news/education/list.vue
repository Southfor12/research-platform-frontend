<template>
  ...
  >
    <el-form :inline="true" :model="queryForm">
      ...
      <el-form-item v-if="checkNode('add_education')" class="right">
    ...
    <el-table :header-cell-style="{ background: '#fafafa' }" :data="list" style="width: 100%">
      ...
      <el-table-column label="类型">
        <template slot-scope="scope">
          ...
          <el-tag v-if="scope.row.type === 6">科学研究</el-tag>
          <el-tag v-if="scope.row.type === 7">教育教学</el-tag>
      ...
      <el-table-column
        v-if="checkNode('edit_education') || checkNode('delete_education')"
        width="180"
        label="操作"
      >
        <template slot-scope="scope">
          <el-button
            v-if="checkNode('edit_education')"
            size="mini"
            type="primary"
          ...
          >
          <el-button
            v-if="checkNode('delete_education') && scope.row.is_deleted === 0"
            size="mini"
            type="danger"
          ...
          >
          <el-button
            v-if="checkNode('delete_education') && scope.row.is_deleted === 1"
            size="mini"
            type="info"
...
export default {
  ...
  methods: {
    ...
    toAdd(id = '') {
      if (!id) {
        this.$router.push({ path: '/education/add' });
      } else {
        this.$router.push({ path: '/education/edit/' + id });
      }
    },
