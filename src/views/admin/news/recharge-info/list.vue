<template>
  ...
  >
    <el-form :inline="true" :model="queryForm">
      ...
      <el-form-item v-if="checkNode('add_recharge_info')" class="right">
    ...
    <el-table :header-cell-style="{ background: '#fafafa' }" :data="list" style="width: 100%">
      ...
      <el-table-column
        v-if="checkNode('edit_recharge_info') || checkNode('delete_recharge_info')"
        width="180"
        label="操作"
      >
        <template slot-scope="scope">
          <el-button
            v-if="checkNode('edit_recharge_info')"
            size="mini"
            type="primary"
          ...
          >
          <el-button
            v-if="checkNode('delete_recharge_info') && scope.row.is_deleted === 0"
            size="mini"
            type="danger"
          ...
          >
          <el-button
            v-if="checkNode('delete_recharge_info') && scope.row.is_deleted === 1"
            size="mini"
            type="info"
...
export default {
  ...
  methods: {
    ...
    toAdd(id = '') {
      if (!id) {
        this.$router.push({ path: '/recharge-info/add' });
      } else {
        this.$router.push({ path: '/recharge-info/edit/' + id });
      }
    },
