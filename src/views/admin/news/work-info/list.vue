    ...
    <el-form :inline="true" :model="queryForm">
      ...
      <el-form-item label="类型">
        <el-select
          ...
          placeholder="类型"
        >
          <el-option label="入室流程" value="16"></el-option>
          <el-option label="仪器培训" value="17"></el-option>
          <el-option label="技术培训" value="18"></el-option>
        </el-select>
      </el-form-item>
      ...
      <el-form-item v-if="checkNode('add_work_info')" class="right">
    ...
    <el-table :header-cell-style="{ background: '#fafafa' }" :data="list" style="width: 100%">
      ...
      <el-table-column label="类型">
        <template slot-scope="scope">
          <el-tag v-if="scope.row.type === 16">入室流程</el-tag>
          <el-tag v-if="scope.row.type === 17">仪器培训</el-tag>
          <el-tag v-if="scope.row.type === 18">技术培训</el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column
        v-if="checkNode('edit_work_info') || checkNode('delete_work_info')"
        width="180"
        label="操作"
      >
        <template slot-scope="scope">
          <el-button
            v-if="checkNode('edit_work_info')"
            size="mini"
            type="primary"
          ...
          >
          <el-button
            v-if="checkNode('delete_work_info') && scope.row.is_deleted === 0"
            size="mini"
            type="danger"
          ...
          >
          <el-button
            v-if="checkNode('delete_work_info') && scope.row.is_deleted === 1"
            size="mini"
            type="info"
...
export default {
  ...
  methods: {
    ...
    toAdd(id = '') {
      if (!id) {
        this.$router.push({ path: '/work-info/add' });
      } else {
        this.$router.push({ path: '/work-info/edit/' + id });
      }
    },
