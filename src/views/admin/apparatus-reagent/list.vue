<template>
  ...
  >
    ...
    <el-table :header-cell-style="{ background: '#fafafa' }" :data="list" style="width: 100%">
      ...
      <el-table-column label="耗材">
        <template slot-scope="scope">
          <span v-for="(item, index) in scope.row.reagents" :key="index">
            {{ item.name }}{{ index === scope.row.reagents.length - 1 ? '' : '、' }}
          </span>
      ...
      <el-table-column v-if="checkNode('edit_apparatus_reagent')" width="180" label="操作">
        <template slot-scope="scope">
          <el-button
            v-if="checkNode('edit_apparatus_reagent')"
            size="mini"
            type="primary"
...
<script>
import { apparatusList, deleteApparatusReagent } from '@/api/apparatus';
...
export default {
  ...
  methods: {
    ...
    handleDelete(row, status) {
      this.$confirm('确定要删除该耗材吗？', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning',
      }).then(() => {
        deleteApparatusReagent({ id: row.id, is_deleted: status }).then((response) => {
    ...
    toAdd(id = '') {
      if (!id) {
        this.$router.push({ path: '/apparatus-reagent/add' });
      } else {
        this.$router.push({ path: '/apparatus-reagent/edit/' + id });
      }
    },
