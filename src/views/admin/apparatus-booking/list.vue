  ...
  >
    <el-form :inline="true" :model="queryForm">
      ...
      <el-form-item label="仪器管理员">
        ...
        >
          <el-option
            v-for="(user, userIndex) in allApparatusAdmins"
            :key="userIndex"
            :label="user.name"
            :value="user.id"
          />
      ...
      <el-form-item label="科研组室">
        ...
        >
          <el-option
            v-for="(platform, platformIndex) in allPlatforms"
            :key="platformIndex"
            :label="platform.name"
            :value="platform.id"
          />
    ...
    <el-table :header-cell-style="{ background: '#fafafa' }" :data="list" style="width: 100%">
      ...
      <el-table-column prop="creator_name" label="预约人">
        <template slot-scope="scope">
          <span>姓名: {{ scope.row.creator_name }}</span
          ><br />
          <span v-if="scope.row.creator_name != scope.row.user_name"
            >操作人: {{ scope.row.user_name }}</span
          >
        </template>
      </el-table-column>
      ...
      <el-table-column label="预约类型">
        <template slot-scope="scope">
          ...
          <span style="color: #33333333">{{scope.row.id}}</span>
        </template>
      </el-table-column>
      <el-table-column prop="discount_charge" label="预计费用" />
      <el-table-column label="实际费用">
        <template slot-scope="scope">
          {{ scope.row.discount_charge }}
          <!-- {{ scope.row.discount_charge +","+ scope.row.overtime_charge }} -->
      ...
      <el-table-column fixed="right" width="180" label="操作">
        <template slot-scope="scope">
          ...
          >
          <el-button
            v-if="checkNode('booking_examine') && scope.row.status === 1"
            size="mini"
            type="success"
          ...
          <div
            class="top-10"
            v-if="
              checkNode('booking_charge') && scope.row.status === 1 && scope.row.is_charge === 0
            "
          >
          ...
          <!-- <div class="top-10" v-if="scope.row.user_id === member.id && scope.row.status < 2 && scope.row.is_charge !== 1">
                    <el-button
                        size="mini"
                        @click="handleCancleBooking(scope.row)"
                    >取消预约</el-button>
                </div> -->
        </template>
      </el-table-column>
...
<script>
import { allUsers, allApparatusAdmins } from '@/api/user.js';
import { allPlatforms } from '@/api/platform';
import { apparatusList, bookingList, bookableUpdate } from '@/api/apparatus';
import { bookingCharge, bookingCancle, bookingCancleByAdmin, exportBooking, getBookingFiles } from '@/api/booking';
...
export default {
  ...
  created() {
    ...
    allApparatusAdmins().then((res) => {
      if (res.status == 200) {
        this.allApparatusAdmins = res.data.users;
      }
    });
  ...
  computed: {
    ...
    getPrice() {
      return (
        ...
        Number(this.chargeForm.append_charge) -
        Number(this.chargeForm.subtract_charge) +
        Number(this.currentBooking.overtime_charge)
      );
    },
  },
  methods: {
    ...
    exportData() {
      ...
      let params = Object.keys(this.queryForm)
        ...
        .join('&');
      let url = process.env.VUE_APP_BASE_API + '/booking/export?from=fronted&' + params;
      window.open(url);
    },
    getApparatus(query = '') {
    ...
    handleCharge(row) {
      ...
      if (row.type === 1) {
        this.chargeForm.timeRange = [
          new Date(row.date + ' 00:00:00'),
          new Date(row.date + ' 23:59:59'),
        ];
        // console.log('rnge is')
        // console.log(row);
        // console.log(this.chargeForm.timeRange);
      }
    ...
    comfirmCharge(form) {
      this.$refs.form.validate((valid) => {
        if (valid) {
          // this.chargeForm.apparatus_boot_at = this.chargeForm.date + ' ' + timeFormat(this.chargeForm.timeRange[
          //     0],
          //   'hh:mm:ss')
          // this.chargeForm.apparatus_down_at = this.chargeForm.date + ' ' + timeFormat(this.chargeForm.timeRange[
          //     1],
          //   'hh:mm:ss')
