<template>
  ...
  >
    ...
    <el-table :header-cell-style="{ background: '#fafafa' }" :data="list" style="width: 100%">
      ...
      <el-table-column v-if="checkNode('edit_apparatus_hcs')" width="180" label="操作">
        <template slot-scope="scope">
          <el-button
            v-if="checkNode('edit_apparatus_hcs')"
            size="mini"
            type="primary"
...
<script>
import { apparatusList, deleteApparatusHcs } from '@/api/apparatus';
...
export default {
  ...
  methods: {
    ...
    toAdd(id = '') {
      if (!id) {
        this.$router.push({ path: '/apparatus-hcs/add' });
      } else {
        this.$router.push({ path: '/apparatus-hcs/edit/' + id });
      }
    },
