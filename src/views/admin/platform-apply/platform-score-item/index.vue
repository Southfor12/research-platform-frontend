<template>
  ...
  >
    <el-form :inline="true">
      <el-form-item>实验室评分项</el-form-item>
      ...
      <el-form-item class="right" v-if="checkNode('add_platform_score_item')">
        <el-button type="primary" size="small" icon="el-icon-plus" @click="toAddType(false)"
          >添加实验室评分项类型</el-button
        >
      </el-form-item>
    </el-form>
        ...
        <template slot="title">
          <div style="width: 100%; display: flex">
            ...
            <div style="display: inline-block; width: 50%; text-align: right; padding-right: 15px">
              <el-button
                size="mini"
                v-if="checkNode('add_platform_score_item')"
              ...
              <el-button
                size="mini"
                v-if="checkNode('delete_platform_score_item')"
                type="danger"
    ...
    <empty v-else description="暂无实验室评分项" />
    <el-dialog
      :title="typeForm.id ? '修改实验室评分项类型' : '添加实验室评分项类型'"
    ...
    <el-dialog
      :title="typeItemForm.id ? '修改实验室评分项' : '添加实验室评分项'"
...
export default {
  ...
  methods: {
    queryList() {
      platformScoreTypeList({ type: 1, size: 200, factory_id: this.factory_id }).then((res) => {
        if (res.status === 200) {
    ...
    toDeleteType(item) {
      this.$confirm('确定要删除实验室评分项类型"' + item.name + '"吗？', '提示', {
    ...
    toDeleteTypeItem(item) {
      this.$confirm('确定要删除实验室评分项"' + item.name + '"吗？', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning',
      }).then(() => {
