<template>
  ...
  >
    <el-form :inline="true">
      <el-form-item>个人评分项</el-form-item>
      ...
      <el-form-item class="right" v-if="checkNode('add_personal_score_item')">
    ...
    <el-collapse v-if="list.length > 0">
      <el-collapse-item v-for="(item, index) in list" :key="index" :name="item.id">
        <template slot="title">
          <div style="width: 100%; display: flex">
            ...
            <div style="display: inline-block; width: 50%; text-align: right; padding-right: 15px">
              <el-button
                size="mini"
                v-if="checkNode('add_personal_score_item')"
                @click.stop="toAddTypeItem(false, item.id)"
                >添加个人评分项</el-button
              >
              <el-button
                size="mini"
                v-if="checkNode('edit_personal_score_item')"
                type="primary"
              ...
              <el-button
                size="mini"
                v-if="checkNode('delete_personal_score_item')"
                type="danger"
        ...
        <el-table :data="item.items" border style="width: 100%">
          ...
          <el-table-column
            label="操作"
            v-if="checkNode('edit_personal_score_item') || checkNode('delete_personal_score_item')"
          >
            <template slot-scope="scope">
              <el-button
                size="mini"
    ...
    <empty v-else description="暂无个人评分项" />
    <el-dialog
      :title="typeForm.id ? '修改个人评分项类型' : '添加个人评分项类型'"
    ...
    <el-dialog
      :title="typeItemForm.id ? '修改个人评分项' : '添加个人评分项'"
...
export default {
  ...
  methods: {
    queryList() {
      platformScoreTypeList({ type: 3, size: 200, factory_id: this.factory_id }).then((res) => {
        if (res.status === 200) {
    ...
    toDeleteType(item) {
      this.$confirm('确定要删除个人评分项类型"' + item.name + '"吗？', '提示', {
    ...
    toDeleteTypeItem(item) {
      this.$confirm('确定要删除个人评分项"' + item.name + '"吗？', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning',
      }).then(() => {
