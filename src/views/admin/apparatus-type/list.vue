<template>
  ...
  >
    ...
    <el-table :header-cell-style="{ background: '#fafafa' }" :data="list" style="width: 100%">
      ...
      <el-table-column
        v-if="checkNode('edit_apparatus_type') || checkNode('delete_apparatus_type')"
        width="180"
        label="操作"
      >
        <template slot-scope="scope">
          <el-button
            v-if="checkNode('edit_apparatus_type')"
            size="mini"
            type="primary"
...
export default {
  ...
  methods: {
    ...
    toAdd(id = '') {
      if (!id) {
        this.$router.push({ path: '/apparatus-type/add' });
      } else {
        this.$router.push({ path: '/apparatus-type/edit/' + id });
      }
    },
