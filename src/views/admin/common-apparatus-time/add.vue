<template>
  ...
  >
    <el-form ref="form" :model="form" :rules="rules" label-width="180px">
      ...
      <el-form-item label="每人每天最多预约次数" prop="max_time_num">
        <el-input-number
          v-model="form.one_day_max_book_count"
          :min="0"
          :step="0.5"
          class="width-200"
        ></el-input-number>
      ...
      <el-form-item label="预约操作时间范围" prop="booking_limit">
        <el-radio v-model="form.booking_limit" :label="0">不限制</el-radio>
        <el-radio v-model="form.booking_limit" :label="1">限制</el-radio>
      </el-form-item>

      <el-form-item label="可进行预约开始时间" prop="booking_start_at" v-if="form.booking_limit==1">
        <el-time-select
          v-model="form.booking_start_at"
          :picker-options="{
            start: '00:00',
            step: '00:30',
            end: '24:00'
      ...
      <el-form-item label="可进行预约结束时间" prop="booking_end_at" v-if="form.booking_limit==1">
        <el-time-select
          v-model="form.booking_end_at"
          :picker-options="{
            start: '00:00',
            step: '00:30',
            end: '24:00'
  ...
  data() {
    ...
    return {
      ...
      form: {
        ...
        max_time_num: 8,//最长可以预约时间
        one_day_max_book_count: 24,//每人每天最多预约次数
        max_day: 90,//可预约范围,默认3个月
        cancel_time_num: 1,//可提前取消时间
        weeks_arr: [],
        book_time: [],
        booking_limit: 0,
        booking_start_at: 0,
      ...
      if (this.form.id) {
        commonApparatusTimeInfo({
          ...
          if (response.status !== 200) {
            ...
            this.form.one_day_max_book_count = data.one_day_max_book_count;
            ...
            this.form.booking_limit = data.booking_limit;
            this.form.booking_start_at = data.booking_start_at;
            this.form.booking_end_at = data.booking_end_at;
    ...
    submit(form) {
      this.$refs[form].validate((valid) => {
        if (valid) {
          ...
          this.form.advance_book_time = this.form.advance_book_time_num * 3600;
          ...
          this.form.weeks = this.form.weeks_arr.sort(sortNumber).join(',');
          this.loading = true;
          if (this.form.id) {
