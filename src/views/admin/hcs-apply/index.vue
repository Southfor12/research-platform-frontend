    ...
    <el-table :header-cell-style="{ background: '#fafafa' }" :data="list" style="width: 100%">
      ...
      <el-table-column width="180" label="操作">
        <template slot-scope="scope">
          ...
          <div
            v-if="
              checkNode('audit_hcs_apply') &&
              scope.row.mentor_audit === 1 &&
              scope.row.status === 0 &&
              ((scope.row.admin_id_1 === member.id && scope.row.admin1_status === 0) ||
                (scope.row.admin_id_2 === member.id && scope.row.admin2_status === 0))
            "
          >
            <el-button size="mini" type="primary" plain @click="handleAudit(scope.row, 1)"
              >通过</el-button
            >
            <el-button size="mini" type="danger" plain @click="handleAudit(scope.row, 2)"
              >拒绝</el-button
            >
          </div>
        </template>
...
<script>
import { chemicalBookingList, auditChemicalBooking } from '@/api/chemical';
...
export default {
  ...
  methods: {
    ...
    handleAudit(row, status) {
      this.auditText = status === 2 ? '确定要拒绝申请吗？' : '提示';
    ...
    comfirmAudit() {
      auditChemicalBooking(this.auditForm).then((response) => {
