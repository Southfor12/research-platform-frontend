<template>
  <div v-loading="loading" class="app-container" element-loading-text="数据处理中..." element-loading-spinner="el-icon-loading">
    <el-form ref="form" :model="form" :rules="rules" label-width="100px">
      ...
      <el-form-item label="课题组" prop="research_group_id">
        <el-select v-model="form.research_group_id" size="small" clearable class="width-200" filterable remote
          reserve-keyword :remote-method="getResearch" :loading="searchLoading" placeholder="输入姓名搜索选择">
          <el-option v-for="item in research" :key="item.id" :label="item.name" :value="item.id" />
        </el-select>
      </el-form-item>
      <el-form-item label="备注">
        <el-input v-model="form.note" size="small" class="width-200" clearable />
      </el-form-item>
      <el-form-item>
  ...
  export default {
    ...
    methods: {
      ...
      submit(form) {
        this.$refs[form].validate((valid) => {
          if (valid) {
            this.loading = true
            if (this.form.id) {
              editResearchGroupAccount(this.form).then(response => {
                this.loading = false
              ...
              addResearchGroupAccount(this.form).then(response => {
                this.loading = false
                if (response.status === 1) {
                  this.returnToList()
                }
      ...
      getResearch(query = '') {
        ...
        researchGroupList(this.searchForm).then(res => {
