<template>
  ...
  >
    <el-form :inline="true" :model="queryForm">
      <el-form-item label="培训" class="width-300">
        <el-select
    ...
    <el-table :header-cell-style="{ background: '#fafafa' }" :data="list" style="width: 100%">
      ...
      <el-table-column label="报名时间" prop="created_at" width="100" />
      <el-table-column prop="factory_id" label="单位">
          <template slot-scope="scope">
            {{getFactoryName(scope.row.factory_id)}}
          </template>
      </el-table-column>
      <el-table-column v-if="checkNode('check_train_enroll')" width="220" label="操作">
        <template slot-scope="scope">
          <el-button
...
export default {
  ...
  created() {
    ...
    this.trainId = localStorage.getItem('train_id') || 0;
    this.trainName = localStorage.getItem('train_name') || '';

    if (this.trainId > 0) {
      this.queryForm.train_id = this.trainId;
      this.trains.push({
        id: this.trainId,
        name: this.trainName,
      });
  ...
  destroyed() {
    localStorage.setItem('train_id', 0);
    localStorage.setItem('train_name', '');
  },
  methods: {
    queryList() {
      this.loading = true;
    ...
    queryTrains(query = '') {
      ...
      trainList(this.searchTrainForm).then((res) => {
        if (res.status === 200) {
          ...
          //   if(this.trains[i].id ==  this.trainId) {
          //     hasTrain = true;
          //     break;
          //   }
          // }
